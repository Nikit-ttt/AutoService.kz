...
function initRegisterFormValidation() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (isValid) {
            ...
            let usersBase = JSON.parse(localStorage.getItem('registeredUsers')) || [];
            const userExists = usersBase.some(user => user.email === email);
            if (userExists) {
            ...
            localStorage.setItem('registeredUsers', JSON.stringify(usersBase));
            const authCard = document.querySelector('.auth-card');
            if (authCard) {
                authCard.innerHTML = `
                    <div style="padding: 20px; text-align: center;">
                        <h2 style="color: #4caf50; margin-bottom: 15px;">Регистрация успешна!</h2>
                        <p style="margin-bottom: 20px;">Добро пожаловать, <strong>${escapeHtml(fullname)}</strong>. Теперь вы можете войти.</p>
